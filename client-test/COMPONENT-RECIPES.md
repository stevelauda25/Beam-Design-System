...
Prerequisites assumed: project is set up per `CLIENT-PROMPT.md` (Tailwind v3 + `@beam/tokens` preset wired up).
...
**Prompt:**
> Build an app header using `@beam/ui`. It should contain: a logo on the left, a search input in the center (max-width 480px), and three icon-only buttons on the right (notifications, settings, sign-out) — each wrapped in a `Tooltip`. Sticky at the top. Use semantic tokens only.

**Code:**

```tsx
import { Button, SearchInput, Tooltip } from '@beam/ui';
...
```tsx
import { Button, SearchInput } from '@beam/ui';
import { useState } from 'react';
...
```tsx
import { Button, Checkbox } from '@beam/ui';
import { useState } from 'react';
...
```tsx
import { Button, Tooltip } from '@beam/ui';
...
```tsx
import { Button, Checkbox, SearchInput } from '@beam/ui';
...
import { Button } from '@beam/ui/dist/button';   // use top-level only
```
