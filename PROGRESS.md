...
|---|---|---|
| `@beam/tokens` | **0.1.2** | `theme.css` (CSS vars light + dark), `tailwind-preset.ts` (utility mapping) |
| `@beam/ui` | **0.1.3** | `Button`, `Checkbox`, `SearchInput`, `Tooltip`, `cn()` helper, `AGENTS.md` manifest |

`AGENTS.md` is shipped in the npm tarball — auto-syncs to consumer's
`node_modules/@beam/ui/AGENTS.md` on `npm update`. Consumer CLAUDE.md
...
4. **Vite cache invalidation gotcha**
   - After `npm update @beam/ui`, vite kept serving old optimized deps.
...
- **Subpath imports** (`@beam/ui/button`) — exposed in exports map but discouraged in client guide. Decide: keep dual-mode or remove subpath exports.
