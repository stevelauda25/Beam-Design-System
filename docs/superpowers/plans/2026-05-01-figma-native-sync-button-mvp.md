...
- **Build command:** `cd ../.. && pnpm install --frozen-lockfile && pnpm -F @beam/tokens build && pnpm -F @beam/ui build && pnpm -F docs build`
...
```bash
pnpm install
pnpm -F @beam/tokens build
pnpm -F @beam/ui build
pnpm -F docs build
```
