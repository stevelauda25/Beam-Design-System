...
- `pnpm --filter @beam/tokens build` clean.
- `pnpm --filter @beam/ui build` clean.
...
- **Centernode and client-test consume `@beam/ui` via npm.** Per CLAUDE.md, local changes here won't show in deployed centernode until `/publish` runs and bumps centernode's dependency. Out of scope for this PR; `/publish` is a separate user-triggered command.
...
- Publishing new `@beam/ui` to npm — separate `/publish` flow.
