---
description: Publish @beam/tokens + @beam/ui to npm. Verify in-sync, build, bump, publish, tag. Sat-set with one safety pause.
---
...
6. **Latest npm version check**: `npm view @beam/ui version` and
   `npm view @beam/tokens version`. Target must be > both. If somebody else
...
workspace symlink for `@beam/tokens` may point at a stale pnpm-cached
...
- `packages/ui/package.json` → `version` AND `dependencies["@beam/tokens"]` (lockstep)
...
═══════════════════════════════════════════════════
  ...
  Files included (from package.json "files"):
    @beam/tokens: dist/, src/theme.css
    @beam/ui:     dist/

  Total package size: <X kB> (estimated via npm pack --dry-run)

  @beam/ui tarball should report 113 files (NOT 112). If 112, dist/styles.css
